...
### 개요
행동경제학의 **심리적 편향 4종(손실회피·시기심·자기고양·체면)**을 내시 바게닝에 통합하여,
두 부처가 심리적으로 수용할 수 있는 최적 예산 배분안을 도출합니다.
...
3. 요청 본문(JSON)에 두 부처의 정보를 입력합니다.
4. **"Execute"** 버튼을 클릭합니다.
5. 하단 **응답(Response)** 에서 최적 배분 결과를 확인합니다.
...
| 파라미터 | 설명 | 예시 |
|----------|------|------|
| `total_budget` | 양 부처에 배분할 총 예산 | `100` |
| `name` | 부처 명칭 | `"기획재정부"` |
| `weight_w` | 국정과제 권한 가중치 (AHP 점수) | `1.2` |
| `disagreement_d` | 협상 결렬 시 손실 수치 | `10` |
| `historical_x0` | 준거점 예산 (전년도 기준) | `50` |
| `bias_lambda` | 손실회피 계수 λ (기본값 2.2) | `2.2` |
| `bias_beta` | 시기심·불평등혐오 계수 β | `0.5` |
| `bias_alpha` | 자기고양 편향 가중치 α (≥1.0) | `1.1` |
| `min_reputation_M` | 최소 체면 마지노선 M | `40` |
| `hostility_AB` | 부처 간 대립도 H (0이면 중립) | `0.2` |
...
| 응답 필드 | 설명 |
|-----------|------|
| `status` | `success` / `infeasible` / `no_equilibrium` |
| `optimized_allocation` | A·B 부처 최적 배분 예산 |
| `psychological_satisfaction` | 각 부처의 심리적 수정 효용값 |
| `coordination_note` | 조정 결과 자연어 해설 |

---

### 상태 코드 의미

- **`success`** : 최적 조정안 도출 성공
- **`infeasible`** : 양측 체면 마지노선 합이 총 예산 초과 → 패키지 딜 필요
- **`no_equilibrium`** : 단일 예산만으로 내시균형 미달 → 2단계 중재 권장
"""
